...
def make_macro(seed):
	...
	os.makedirs('./product/'+run_macro_name+'/'+output_dirname, exist_ok=True)
	fout = open('./product/'+run_macro_name+'/'+output_dirname+'/s'+str(seed).zfill(4)+'.mac', mode='w')
...
def make_shell(seed):
	...
	os.makedirs('./product/'+run_macro_name+'/'+output_dirname, exist_ok=True)
	foutname = './product/'+run_macro_name+'/'+output_dirname+'/s'+str(seed).zfill(4)+'.sh'
	...
	for line in fin:
		...
		if 0 == line.find('    -n') :
			...
			fout.write('    -f '+job_maker_dir+'/product/'+run_macro_name+'/macro/s'+str(seed).zfill(4)+'.mac \\\n')
			...
			#fout.write('#SBATCH --output='+job_maker_dir+'/product/'+run_macro_name+'/log/s'+str(seed).zfill(4)+'.o \n')
			fout.write('#SBATCH --output='+workdir+'/s'+str(seed).zfill(4)+'.o \n')
		elif 0 == line.find('#SBATCH --error=') :
			#fout.write('#SBATCH --error='+job_maker_dir+'/product/'+run_macro_name+'/log/s'+str(seed).zfill(4)+'.e \n')
	...
	#os.makedirs('./product/'+run_macro_name+'/'+'log', exist_ok=True)


def make_throw(NofBatchs):

	foutname = './product/'+run_macro_name+'/throw.sh'
	fout = open(foutname, mode='w')

	fout.write('#! /bin/bash \n')
	for i in range(1,NofBatchs+1):
		batchpath = job_maker_dir+'/product/'+run_macro_name+'/shell/s'+str(i).zfill(4)+'.sh'
